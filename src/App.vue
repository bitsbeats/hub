      ...
      <b-tab-item label="Tags" :visible="tagTabVisible">
        <p class="control">
          <b-button
            ...
            size="is-small"
            @click="deleteTag"
          >Delete {{ tagsChecked.length }} tags</b-button>
  ...
  methods: {
    ...
    deleteTagConfirmed() {
      this.tagsChecked.map(item => {
        this.$http
          .get(`/v2/${item.image}/manifests/${item.tag}`, {
            headers: {
          ...
          .then(({ headers }) => {
            ...
            this.$http
              .delete(`/v2/${item.image}/manifests/${digest}`)
              .then(() => {
    ...
    loadImages() {
      this.images = [];
      this.loadImagesSubpage("/v2/_catalog");
    },
    loadImagesSubpage(url) {
      this.$http.get(url).then(({ headers, data }) => {
        if (headers.link) {
          const nexturl = /<([^>]+)>; rel="next"/.exec(headers.link);
          if (nexturl) {
            this.loadImagesSubpage(nexturl[1]);
          }
        }
    ...
    loadInfo(tag) {
      this.$http
        .get(`/v2/${tag.image}/manifests/${tag.tag}`)
        .then(({ data }) => {
          if ("history" in data && 0 in data.history) {
            ...
            if (
              tagInfo["container_config"] &&
              tagInfo["container_config"]["Labels"]
            ) {
    ...
    updateInfo() {
      var keys = Object.keys(this.infoSortMap)
        .sort()
        .reverse();
      var infos = [];
      keys.map(key => {
